...
class MaradigmaBookingModal {
  ...
  async next() {
    ...
    if (step === 1 || step === 2) {
      try {
        ...
        this._showErrorAndScroll(e?.message || this._t("errors.bookingError", "Booking error"));
        console.groupEnd();
        return;
      } finally {
        this._bookingStepBusy = false;
  ...
  async _syncBookingOnlineStep(step) {
    ...
    const res = await this.api.postJson(this.cfg.bookingOnlineEndpoint, payload);
  ...
  _renderOrUpdateSkipperPrompt() {
    ...
    slot.innerHTML = `
      <div class="md-mt-8 md-skipper-prompt">
        <label>${this._t("labels.hireSkipperTitle", "Would you like a professional skipper on board?")}</label>
        <div class="md-muted">
          ${this._t(
  ...
  _renderStep1() {
    ...
    const dateFieldHtml = isInlineCalendar
      ? `
        <div class="md-col-12 md-field md-field--calendar-inline">
          <label>${this._t("labels.dateRange", isSingleMode ? "Date" : "Dates")}</label>
    ...
    return `
      <div class="md-step1-layout">
        <div class="md-step1-main">
          <div class="md-grid">
            ${dateFieldHtml}
            ...
            <div class="md-col-12 md-field">
              <label>${this._t("labels.people", "People")}</label>
              ${peopleField}
            </div>
            ...
            ${shouldShowChildrenBlock ? `
              ...
              <div class="md-col-12 md-field" data-md-children-wrap="1" style="${showChildrenSelect ? "" : "display:none;"}">
                <label>${this._t("labels.chooseChildren", "Choose the number of children")}</label>
            ...
            <div class="md-col-12 md-field">
              <label>${this._t("labels.optionalMessage", "Optional message")}</label>
              <div class="md-muted">
...
`;
  ...
  _renderStep2() {
    return `
      <div class="md-grid">
        ...
        <div class="md-col-6 md-field">
          <label>${this._t("labels.phone", "Phone")}</label>
  ...
  _bindStep3Events(containerEl) {
    ...
    if (toggle && !toggle.__mdBound) {
      toggle.addEventListener("click", async (e) => {
        ...
        const termsContentEl = this.stepContainer?.querySelector('[data-md-terms-content="1"]');
        if (!termsContentEl) {
          return;
        }
  ...
  _validateStep(step) {
    ...
    if (step === 1) {
      ...
      const activeRange = this._getActivePriceRangeFromPriceOnBooking();
      const timeslots = Array.isArray(activeRange?.timeslots) ? activeRange.timeslots : null;

      if (Array.isArray(timeslots) && timeslots.length > 0) {
        const canFullDay = timeslots.every((ts) => ts?.compatible_with_day_rental === true);

        if (!this.state.form.full_day) {
          if (!this.state.form.timeslot_id) {
            this._showFieldErrorAndScroll(
  ...
  async _hydrateBoatOnOpen() {
    ...
    if (cached && now - cached.ts < this._boatCacheTtlMs) {
    ...
    try {
      ...
      const raw = await this.api.getJson(url, { nonce: this.cfg.restNonce });
  ...
  _renderTimeslotSelectorIfNeeded() {
    ...
    const unavailableTimeSlots = this._getUnavailableTimeSlotsForDate(ds);

    const timeslots = rangeTimeslots.filter((ts) => {
      const tsId = this._toInt(ts?.id ?? ts?.id_time_slot ?? 0, 0);
      const tsStart = String(ts?.start_time || "").trim();
      const tsEnd = String(ts?.time_end || "").trim();

      const isBlocked = unavailableTimeSlots.some((blocked) => {
        const blockedId = this._toInt(blocked?.id_time_slot ?? 0, 0);
        const blockedStart = String(blocked?.time_start || "").trim();
        const blockedEnd = String(blocked?.time_end || "").trim();

        if (blockedId > 0 && tsId > 0) {
          return blockedId === tsId;
        }

        if (blockedStart && tsStart && blockedEnd && tsEnd) {
          return blockedStart === tsStart && blockedEnd === tsEnd;
        }

        if (blockedStart && tsStart) {
          return blockedStart === tsStart;
        }
    ...
    if (!canFullDay) {
      this.state.form.full_day = false;
      return this._renderTimeslotSelect(timeslots, { showHelp: true, showFullDayToggle: false });
    }
    ...
    const selectHtml = this._renderTimeslotSelect(timeslots, { showHelp: false, showFullDayToggle: false });
  ...
  _renderTimeslotSelect(timeslots, { showHelp = true } = {}) {
    ...
    const placeholder = this._t("ui.selectOneTimeslot", "Select a schedule…");
    const current = this.state.form.timeslot_id ? String(this.state.form.timeslot_id) : "";

    const optionsHtml =
      `<option value="" ${current === "" ? "selected" : ""} disabled>${this._esc(placeholder)}</option>` +
      timeslots
        .map((ts) => {
          const id = ts?.id;
          if (!id) return "";
          const text = this._buildTimeslotLabel(ts, { useAmPm: true });
          const selected = current !== "" && String(id) === current ? "selected" : "";
          return `<option value="${this._esc(String(id))}" ${selected}>${this._esc(text)}</option>`;
        })
        .join("");

    return `
    <div class="md-field">
      <label class="md-label">${this._esc(label)}</label>

      <select class="md-select" data-md-field="timeslot_id" data-md-timeslot-select="1">
        ${optionsHtml}
      </select>

      ${showHelp ? `<div class="md-muted md-mt-8">${this._esc(help)}</div>` : ""}
    </div>
  `;
  }
